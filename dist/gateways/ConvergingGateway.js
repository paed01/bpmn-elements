...
exports.ConvergingGatewayBehaviour = ConvergingGatewayBehaviour;
...
function ConvergingGateway(Behaviour, activityDef, context) {
  if (context.getInboundSequenceFlows(activityDef.id).length < 2) return new _Activity.Activity(Behaviour, activityDef, context);
 ...
 */
function ConvergingGatewayBehaviour(activity) {
  this.id = activity.id;
...
ConvergingGatewayBehaviour.prototype.setup = function setup(executeMessage) {
  ...
  this.inbound.add((0, _messageHelper.cloneContent)(inboundFlows[0]));
