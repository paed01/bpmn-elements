  ...
  function execute(startMessage) {
    ...
    function getTimers() {
      ...
      return cloneContent(messageContent, {
        ...(resolvedTimeDuration ? {timeDuration: resolvedTimeDuration} : undefined),
        ...(resolvedTimeCycle ? {timeCycle: resolvedTimeCycle} : undefined),
        ...(resolvedTimeDate ? {timeDate: timeDateDt || resolvedTimeDate} : undefined),
        ...duration,
        startedAt,
        state: 'timer',
      });
      ...
      function resolveDuration() {
        const {timeout: isoTimeout, isoDuration} = resolveIsoDuration(resolvedTimeDuration);
        const expireAt = resolvedTimeDate && parseDate(resolvedTimeDate);
        const {timeout: dateTimeout} = resolveDateDuration(expireAt);
      ...
      function resolveIsoDuration(isoDuration) {
        ...
        try {
          var timeout = 'timeout' in messageContent ? messageContent.timeout : toSeconds(parse(isoDuration)) * 1000; // eslint-disable-line no-var
        } catch (err) {
    ...
    function completed(completeContent, options) {
      ...
      broker.publish('event', 'activity.timeout', cloneContent(completedContent, completeContent), options);
      broker.publish('execution', 'execute.completed', cloneContent(completedContent, completeContent), options);
    }
