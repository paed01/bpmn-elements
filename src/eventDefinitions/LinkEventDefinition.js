...
export default function LinkEventDefinition(activity, eventDefinition) {
  ...
  const signalQueueName = `link-${brokerSafeId(id)}-${brokerSafeId(reference.linkName)}-q`;
  ...
  function executeCatch(executeMessage) {
    ...
    function onSignalApiMessage(routingKey, message) {
      if (getPropertyValue(message, 'content.originalMessage.linkName') !== reference.linkName) return;
      completed = true;
      stop();
