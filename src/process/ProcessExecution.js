...
proto.execute = function execute(executeMessage) {
  ...
  const stateMessage = this[stateMessageSymbol] = cloneMessage(executeMessage);
...
proto.resume = function resume() {
  ...
  this.activityQ.consume(this.onChildMessage, {prefetch: 1000, consumerTag: `_process-activity-${this.executionId}`});
  ...
  for (const {content} of this.postponed.slice()) {
...
proto.start = function start() {
  ...
  const executeContent = {...this[stateMessageSymbol].content, state: this.status};

  this.broker.publish(this.exchangeName, 'execute.start', cloneContent(executeContent));
  ...
  this.activityQ.assertConsumer(this.onChildMessage, {prefetch: 1000, consumerTag: `_process-activity-${this.executionId}`});
};
...
proto.activate = function activate() {
  this.broker.subscribeTmp('api', '#', this.onApiMessage.bind(this), {noAck: true, consumerTag: `_process-api-consumer-${this.executionId}`, priority: 200});
...
proto.onActivityEvent = function onActivityEvent(routingKey, activityMessage) {
  ...
  switch (routingKey) {
    case 'process.terminate':
      return this.activityQ.queueMessage({routingKey: 'execution.terminate'}, cloneContent(content), {type: 'terminate', persistent: true});
    case 'activity.stop':
      return;
  }

  this.activityQ.queueMessage(message.fields, cloneContent(content), {persistent: true, ...message.properties});
};
...
proto.onChildMessage = function onChildMessage(routingKey, message) {
  ...
  switch (routingKey) {
    case 'activity.detach': {
      this.elements.detachedActivities.push(cloneMessage(message));
      break;
    }
    ...
    case 'activity.error': {
      ...
      return this.complete('error', {error: content.error});
    }
  }
};
...
proto.stopExecution = function stopExecution(message) {
  ...
  return this.broker.publish(this.exchangeName, `execution.stopped.${this.executionId}`, {
...
proto.complete = function complete(completionType, content = {}) {
  ...
  return broker.publish(this.exchangeName, `execution.${completionType}.${this.executionId}`, {
    ...this[stateMessageSymbol].content,
...
proto.discard = function discard() {
  ...
  return this.activityQ.queueMessage({routingKey: 'execution.discard'}, {
    id: this.id,
    type: this.type,
...
proto.getState = function getState() {
  const {flows, outboundMessageFlows, associations} = this.elements;
  return {
    executionId: this.executionId,
...
proto.getApi = function getApi(message) {
  if (!message) return ProcessApi(this.broker, this[stateMessageSymbol]);
...
proto.onShookEnd = function onShookEnd(message) {
  ...
  this.elements.startSequences[message.content.id] = cloneMessage(message);
};
