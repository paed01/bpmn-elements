...
export class AdHocSubProcessBehaviour extends SubProcessBehaviour {
  ...
  completionMet(message, execution) {
    ...
    return !!execution.environment.resolveExpression(this.completionCondition, cloneMessage(message));
  }
...
export function AdHocSubProcess(activityDef, context) {
  return SubProcess({ ...activityDef, isAdHoc: true }, context, AdHocSubProcessBehaviour);
}
