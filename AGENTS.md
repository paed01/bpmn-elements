...
- **Postponed content declares `accepts`.** Every wait-state publish (`activity.wait`, `activity.timer`, `activity.call`, the compensate `execute.detach`) stamps an `accepts` list with the api message types the waiting run acts on beyond the universal `stop`/`discard` — e.g. `['signal', 'error']` for `SignalTask`, `['cancel']` for timers, `['signal', 'cancel']` for conditional events, `['compensate']` for a compensation listener (dismissed by discard, not cancel). It is host-facing contract (documented in `docs/SharedApi.md`, surfaced via `getPostponed()` api content), so when an api handler gains or loses a case, update the corresponding stamp — and vice versa. `CancelEventDefinition` intentionally stamps nothing: it is completed by the attached transaction, not by api.
- **Parallel and inclusive gateways share the converging machinery.** Both are built by `ConvergingGateway` in `src/gateways/ConvergingGateway.js`, which stamps the `isConvergingGateway` flag that drives the shake (`_onShakeMessage`), the start-up peer discovery (`ProcessExecution._activate`), and the inbound-token pop on completion (`_onChildCompleted`). `isParallelGateway` stays a parallel-only marker. Flags are copied into every run message and thus persisted, so a process-side check that reads a flag off message content must also accept the legacy flag of states saved before it existed — `_onChildCompleted` accepts `isParallelGateway` for that reason. `ParallelGatewayBehaviour` and `InclusiveGatewayBehaviour` both extend the internal `ConvergingGatewayBehaviour`; the inclusive one only overrides `_getCompletedContent` to stamp `requireOutbound`. A parallel or inclusive gateway with fewer than two incoming sequence flows has nothing to converge: `ConvergingGateway` returns a plain `Activity` (no `isConvergingGateway` flag, no peers, the shake passes through it) and the behaviour completes on every inbound token, so an uncontrolled merge upstream fires it once per token. A process without converging gateways therefore skips the start-up shake entirely. A recovered state saved mid-monitoring by such a gateway still carries `isConvergingGateway` in its run content, so the process pops its inbound on completion while the behaviour completes immediately (see `test/resources/inclusive-gateway-icicle-state-18.json`).
...
- BPMN sources: raw XML templates in `test/helpers/factory.js` (helpers like `factory.valid()`, `factory.userTask()`, `factory.resource('name')`) plus `.bpmn` files under `test/resources/`. Resource files must carry diagram interchange (`bpmndi:BPMNDiagram` with shapes for every flow node and edges for every flow) so they open and display in a modeler (bpmn-io/Camunda Modeler); inline sources in test files may skip DI.
...
- **No `setTimeout` in tests.** Test performance is key and timers make tests slow and flaky. Drive asynchronous branches through the service function callback instead: hold `next` and call it when the scenario is ready (e.g. once every expected service task has called in, or on a broker event such as `activity.converge`), or use a service that never calls back to keep a branch running until stopped.
