...
- **Parallel and inclusive gateways share the converging machinery.** Both are built by `ConvergingGateway` in `src/gateways/ParallelGateway.js`, which stamps the `isConvergingGateway` flag that drives the shake (`_onShakeMessage`), the start-up peer discovery (`ProcessExecution._activate`), and the inbound-token pop on completion (`_onChildCompleted`). `isParallelGateway` stays a parallel-only marker. Flags are copied into every run message and thus persisted, so a process-side check that reads a flag off message content must also accept the legacy flag of states saved before it existed — `_onChildCompleted` accepts `isParallelGateway` for that reason. `InclusiveGatewayBehaviour` extends `ParallelGatewayBehaviour` and only overrides `_getCompletedContent` to stamp `requireOutbound`.
